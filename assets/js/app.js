...
(function () {
  ...
  // The directory this deck is served from, for the canonical link and the
  // paths the tracks live at. Computed from the address so the same files
  // work at the root of a site or under any folder — GitHub Pages included.
  function pageBase() {
    var p;
    try { p = decodeURIComponent(location.pathname); } catch (e) { p = location.pathname; }
    var owned = /(?:^|\/)(?:playlist|podcast)(?:\/|$)/;
    var m = owned.exec(p);
    var base = m ? p.slice(0, m.index) : p;
  ...
  // tools/build.py.
  var CANON = location.origin;

  // The tracks live in the repo so they can arrive as files.
  var TRACKS_DIR = BASE + 'tracks/';
  var TRACKS_MANIFEST = BASE + 'tracks/playlist.json';
  var LYRICS_DIR = BASE + 'tracks/lyrics/';
  ...
  var CANVAS_W = 1180;
  var CANVAS_H = 880;
  var STORE_KEY = 'radio-skin';
  var STORE_TRACKS = 'radio-playlist';
  var STORE_STORIES = 'radio-stories';
  var STORE_PREFS = 'radio-prefs';
  ...
  var STATION = { name: 'radio', tag: 'playlist' };
  ...
  try {
    ...
    if (prefs && typeof prefs.shuffle === 'boolean') S.shuffle = prefs.shuffle;
    if (prefs && (prefs.repeat === 'all' || prefs.repeat === 'one')) S.repeat = prefs.repeat;
  } catch (e) { /* private mode */ }
  ...
  function makeAudio(analysed) {
    ...
    // the start of the first. That is the whole of the rotation. With the
    // deck set on one song, the start of the next is the start of this one.
    a.addEventListener('ended', function () {
      if (!mine()) return;
      if (S.repeat === 'one') { restartSame(); return; }
      next();
    });
  ...
  function next() {
    ...
    if (S.shuffle) {
      if (l.length === 1) { restartSame(); return; }
      var n;
      do { n = Math.floor(Math.random() * l.length); } while (n === S.ti);
      playFrom(l, S.mode, n);
      return;
    }
    playFrom(l, S.mode, (S.ti + 1) % l.length);
  }

  function restartSame() {
    var want = wantedSrc();
    if (!want) { autostart(); return; }
    // A stopped deck (loadedSrc cleared) starts the same song again; a
    // paused one just rewinds and carries on.
    if (loadedSrc !== want) { play(want, S.mode, S.ti); return; }
    audio.currentTime = 0;
    S.cur = 0;
    intent = 'play';
  ...
  function savePrefs() {
    try { localStorage.setItem(STORE_PREFS, JSON.stringify({ shuffle: S.shuffle, repeat: S.repeat })); }
    catch (e) { /* private mode */ }
  }
  ...
  function showName() { return (S.show && S.show.name) || 'radio stories'; }
  ...
  function paintPrefs() {
    var on = 'is-on', off = '';
    el.shuffleBtn.classList.toggle(on, S.shuffle);
    el.shuffleBtn.setAttribute('aria-pressed', S.shuffle ? 'true' : 'false');
    el.shuffleBtn.title = S.shuffle ? 'shuffle on' : 'shuffle off';
    var one = S.repeat === 'one';
    el.repeatBtn.classList.toggle(on, S.repeat !== 'off');
    el.repeatBtn.classList.toggle('rep-one', one);
    el.repeatBtn.setAttribute('aria-pressed', S.repeat !== 'off' ? 'true' : 'false');
    el.repeatBtn.textContent = '\u21bb';
    el.repeatBtn.title = S.repeat === 'off' ? 'repeat off' : S.repeat === 'all' ? 'repeat all' : 'repeat one';
  }
  ...
  function permalink(t) { return location.origin + BASE + t.key; }
  ...
  function parseRoute(pathname, hash) {
    ...
    // Strip the subpath the site is hosted under, if any: /radio/playlist/song
    // becomes /playlist/song before we examine it.
  ...
  function wantedPath() {
    ...
    if (chose && it && it.kind === showing) return BASE + it.key;
    return S.route ? BASE + S.route : BASE;
  }
